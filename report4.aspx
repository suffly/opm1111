...
<script runat="server">
    ...
    Sub BindData()
        ...
        strSQL = "select if(createdate is null, 'รวมทั้งสิ้น', createdate) as createdate ,total, ctype1, ctype2, nocontact"
        ...
        strSQL = strSQL & " count(CASE WHEN case_inbox.contact= 'T' and customer_type.id = '1' then 1 END) AS ctype1,"
        strSQL = strSQL & " count(CASE WHEN case_inbox.contact= 'T' and customer_type.id = '2' then 1 END) AS ctype2,"
        strSQL = strSQL & " count(CASE WHEN case_inbox.contact= 'F' then 1 END) AS nocontact"
        strSQL = strSQL & " FROM"
        strSQL = strSQL & " case_inbox"
        strSQL = strSQL & " LEFT OUTER JOIN contact ON case_inbox.contact_id = contact.id"
        strSQL = strSQL & " LEFT OUTER JOIN customer_type ON contact.customer_type = customer_type.id"
  ...
  <div class="progress-bar progress-bar-striped progress-bar-animated"  role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width:100%"></div>
    ...
    <!-- Content-->         
     ...
     <div class="d-flex justify-content-between">
            <h2><i class="bi bi-journal-text"></i> 4. รายงานตามผู้ขอใช้บริการ</h2>
                </div>
  </div>
  <div class="card-body">
      ...
      <asp:GridView ID="gridview1" runat="server" AutoGenerateColumns="false" CssClass="table table-hover table-sm">
          <Columns>
              ...
              <asp:TemplateField HeaderText="จำนวน"  HeaderStyle-CssClass="bg-success text-white text-center">
               ...
               <asp:TemplateField HeaderText="ประชาชน"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("ctype1") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>
               ...
               <asp:TemplateField HeaderText="สมาชิกสภาผู้แทนราษฎร"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("ctype2") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>

              <asp:TemplateField HeaderText="ไม่ระบุ"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("nocontact") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>
