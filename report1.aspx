...
<script runat="server">
    ...
    Sub BindData()
        ...
        strSQL = "select if(createdate Is null, 'รวมทั้งสิ้น', createdate) as createdate ,total, obj1, obj2, obj3 "
        strSQL = strSQL & " from(select"
        strSQL = strSQL & " date_format(create_date,'%Y-%m-%d') AS CreateDate, "
        strSQL = strSQL & " Count(case_id) As Total, "
        strSQL = strSQL & " count(CASE WHEN case_objective = '1' then 1 END) AS obj1,"
        strSQL = strSQL & " count(CASE WHEN case_objective = '2' then 1 END) AS obj2,"
        strSQL = strSQL & " count(CASE WHEN case_objective = '3' then 1 END) AS obj3"
        strSQL = strSQL & " From case_inbox"
        strSQL = strSQL & " Where enable ='T' "
        If start_date <> "" And end_date <> "" Then
            strSQL = strSQL & " and date_format(create_date,'%Y-%m-%d') between '" & start_date & "' and '" & end_date & "'"
        End If
        strSQL = strSQL & " Group by createDate asc With rollup) As t"
  ...
  <div class="progress-bar progress-bar-striped progress-bar-animated"  role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width:100%"></div>
    ...
    <!-- Content-->         
     ...
     <div class="d-flex justify-content-between">
            <h2><i class="bi bi-journal-text"></i> 1. รายงานตามประเภทหนังสือ</h2>
                </div>
  </div>
  <div class="card-body">
      ...
      <asp:GridView ID="gridview1" runat="server" AutoGenerateColumns="false" CssClass="table table-hover table-sm">
          <Columns>
              ...
              <asp:TemplateField HeaderText="วันที่บันทึกข้อมูล"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# If(IsDate(Eval("createdate")), FormatDateTime(Eval("createdate"), 2), "รวมทั้งสิ้น") %>' ></asp:Label>
                  </ItemTemplate>
              ...
              <asp:TemplateField HeaderText="ข้อร้องทุกข์"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("obj1") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>

              <asp:TemplateField HeaderText="ข้อหารือ"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("obj2") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>

              <asp:TemplateField HeaderText="แสดงความคิดเห็น"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
                      <asp:Label ID="Label1" runat="server" Text='<%# Eval("obj3") %>'></asp:Label>
                  </ItemTemplate>
                  <itemstyle HorizontalAlign="center" Width="15%" />
              </asp:TemplateField>

               <asp:TemplateField HeaderText="หมายเหตุ"  HeaderStyle-CssClass="bg-success text-white text-center">
                  <ItemTemplate>
